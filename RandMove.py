...
dotsN = 50
fieldSize = 10  # 3x3 square dot field
shapeFieldSize = 5
elemSize = 0.25
speed =  0.01 #13/60 # 7 degree/seconds
posX = 9.06
posY = 0
...
def randomDotMove(randDotsX, randDotsY, randDots, veloX, veloY, outfieldDotsY, outfieldDotsX, deathDots):
    ...
    #randDotsX[shapeInFieldX] += (speed*5) * cos(pi/2) #cos(alpha[shapeInFieldX])
    #randDotsY[shapeInFieldY] += (speed*5) * sin(pi/2)#sin(alpha[shapeInFieldY])

    randDotsX[shapeIn] += (speed*5) * cos(pi/2)
    randDotsY[shapeIn] += (speed*5) * sin(pi/2)
